...
"=============================================================================
"
" Fichier de configuration VIM personnalis?? (meilleur pour la programmation,
...
if filewritable(expand("~/.vim/backup")) == 2
    " comme le r??pertoire est accessible en ??criture,
    " on va l'utiliser.
        set backupdir=$HOME/.vim/backup
else
        if has("unix") || has("win32unix")
        " C'est c'est un syst??me compatible UNIX, on
        " va cr??er le r??pertoire et l'utiliser.
...
" Quand une fermeture de parenth??se est entr??e par l'utilisateur,
" l'??diteur saute rapidement vers l'ouverture pour montrer o?? se
" trouve l'autre parenth??se. Cette fonction active aussi un petit
...
" Tout ce qui concerne la recherche. Incr??mentale
...
" la peine de la refaire depuis le d??but du fichier
set hlsearch
...
" D??sactiver le wrapping
set nowrap

" Options folding
set foldmethod=marker

" Un petit menu qui permet d'afficher la liste des ??l??ments
" filtr??s avec un wildcard
set wildmenu
...
" ="" pas de souris par d??faut
"set mouse=a

" Am??liore l'affichage en disant ?? vim que nous utilisons un terminal rapide
set ttyfast

" Lazy redraw permet de ne pas mettre ?? jour l'??cran
" quand un script vim est entrain de faire une op??ration
set lazyredraw
...
" marche m??me quand le fichier est apr??s
" le caract??re ??gal. Comme :
" variable=/etc/<C-XF>
set isfname-==
...
if has("autocmd")
    ...
    augroup divers " {{{2
        au!
                ...
                " Remet la position du curseur comme elle ??tait avant
                autocmd BufReadPost *
                \ if line("'\"") > 0 && line("'\"") <= line("$") |
                \   exe "normal g`\"" |
                \ endif

        " La valeur des tabs par d??faut
...
" Fonctions utilis??e par vimrc {{{2
...
" Aller dans le r??pertoire du fichier ??dit??.
...
" Entrer la commande ":e" dans le r??pertiore du fichier ??dit??
if has("unix")
...
" Copier (le gv c'est pour remettre le s??lection)
map <C-c> "+ygv
...
" D??sactiver le highlight (lors d'une recherche par exemple)
map <F2> :let @/=""<cr>
